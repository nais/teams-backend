...
func (s *gitHubReconciler) connectUsers(ctx context.Context, githubTeam *github.Team, corr dbmodels.Correlation, team dbmodels.Team) error {
	...
	for _, username := range missing {
		...
		s.auditLogger.Logf(OpAddMember, corr, s.system, nil, &team, nil, "added member '%s' to GitHub team '%s'", username, *githubTeam.Slug)
	}
	...
	for _, username := range extra {
		...
		// TODO: Connect local database user to log entry
		s.auditLogger.Logf(OpDeleteMember, corr, s.system, nil, &team, nil, "deleted member '%s' to GitHub team '%s'", username, *githubTeam.Slug)
	}
