...
func (ec *executionContext) __resolve_entities(ctx context.Context, representations []map[string]interface{}) []fedruntime.Entity {
	...
	resolveEntity := func(ctx context.Context, typeName string, rep map[string]interface{}, idx []int, i int) (err error) {
		...
		switch typeName {
		case "GcpProject":
			resolverName, err := entityResolverNameForGcpProject(ctx, rep)
			if err != nil {
				return fmt.Errorf(`finding resolver for Entity "GcpProject": %w`, err)
			}
			switch resolverName {

			case "findGcpProjectByProjectID":
				id0, err := ec.unmarshalNString2string(ctx, rep["projectId"])
				if err != nil {
					return fmt.Errorf(`unmarshalling param 0 for findGcpProjectByProjectID(): %w`, err)
				}
				entity, err := ec.resolvers.Entity().FindGcpProjectByProjectID(ctx, id0)
				if err != nil {
					return fmt.Errorf(`resolving Entity "GcpProject": %w`, err)
				}
...
func entityResolverNameForGcpProject(ctx context.Context, rep map[string]interface{}) (string, error) {
	for {
		var (
		...
		return "findGcpProjectByProjectID", nil
	}
	return "", fmt.Errorf("%w for GcpProject", ErrTypeNotFound)
}
